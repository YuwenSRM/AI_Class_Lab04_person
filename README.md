...
一个使用原生 HTML、CSS 和 JavaScript 构建的个人作品集页面，用于在桌面端和移动端展示个人介绍、技能方向与项目作品。

## 项目特点

- 采用左侧个人信息与导航、右侧纵向作品展示的响应式布局；在移动端自动切换为顶部栏与单栏内容。
- 项目数据集中维护在 `js/projects.js`，可自动生成项目列表、类别筛选、编号和交错图文布局。
- 每个项目展示类别、标签、完成时间、简介、技术栈和项目封面，当前包含移动应用、Web 应用、数据可视化和 AI 应用等方向。
- 提供日间 / 夜间模式切换，主题偏好通过 `localStorage` 保存。
- 包含移动端导航、当前区块高亮、邮箱复制与返回顶部等基础交互。

## 技术说明

项目不依赖构建工具、前端框架或第三方 UI 组件库：页面结构、样式和交互分别维护在 HTML、CSS 和 JavaScript 文件中。Google Fonts 仅用于字体加载，网络不可用时会自动回退到系统字体。
